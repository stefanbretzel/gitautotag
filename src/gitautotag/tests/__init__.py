    ...
    def tearDown(self):
        if os.path.exists(self.tempdir):
            if not os.path.isdir(self.tempdir):
                raise Exception("Expected {0} to be "
                                "a directory.".format(self.tempdir))
...
class GitRepoWithRemoteTestCase(GitRepoTestCase):
    ...
    def setUp(self):
        if getattr(self, 'do_skip', False):
            raise SkipTest("Test cannot be run from setup.py")
        self.tempdir = tempfile.mkdtemp()
        ...
        self.local_repo = Repo.clone_from(self.remote_tempdir,
                                          self.local_tempdir)
